...
function categoryNode(p: any) {
  return p.model?.kategoria || p.egzemplarz?.model?.kategoria || null;
}

function cat(p: any) {
  let category = categoryNode(p);
  if (!category) return 'Bez kategorii';

  // PDF grupuje sprzęt według kategorii głównej.
  // Backend musi zwrócić relację kategoria.rodzic (patrz zmiana w magazyn.service.ts).
  while (category?.rodzic) category = category.rodzic;

  return category?.nazwa || 'Bez kategorii';
}
...
export default function WarehouseDocumentPdfPage() {
  ...
  const isRental = Boolean(doc.id_wynajmu || doc.wynajem);
  const contextTitle = isRental ? 'Wypożyczenie' : 'Wydarzenie';
  const actionTitle = doc.typ === 'przyjecie' ? 'Zdanie' : 'Odbiór';
  ...
  return <div className="eventflow-pdf-page min-h-screen bg-slate-100 p-6 print:bg-white print:p-0">
    ...
    <main className="eventflow-pdf-sheet mx-auto min-h-[297mm] w-[210mm] max-w-full bg-white p-[12mm] text-slate-900 shadow-xl print:shadow-none">
      <header className="eventflow-pdf-header mb-10 grid grid-cols-2 gap-8">
        <div>
          <img src={doc.organizacja?.logo || "/eventflow-logo.svg"} alt={doc.organizacja?.nazwa || "Logo firmy"} className="mb-6 max-h-16 max-w-[260px] object-contain object-left" />
          {/* <p className="text-sm font-bold">{doc.organizacja?.nazwa || 'EventFlow'}</p> 
          <p className="text-xs text-slate-500">Dokument magazynowy wygenerowano w systemie EVE-nt.</p>*/}
        </div>
      ...
      <section className="eventflow-pdf-meta mb-8 grid grid-cols-2 gap-8 text-sm">
        <div>
          <h2 className="mb-2 border-b pb-1 font-black uppercase text-slate-500">{contextTitle}</h2>
          <p className="font-black">
            {isRental
              ? (doc.wynajem?.numer ? `Wypożyczenie ${doc.wynajem.numer}` : 'Wypożyczenie')
              : (doc.wydarzenie?.nazwa || '-')}
          </p>
          <p>{doc.wydarzenie?.kontrahent?.nazwa || doc.wynajem?.kontrahent?.nazwa || ''}</p>
        </div>
        <div>
          <h2 className="mb-2 border-b pb-1 font-black uppercase text-slate-500">{actionTitle}</h2>
          <p>{doc.id_wynajmu ? 'Osoba:' : 'Z konta:'} <b>{doc.id_wynajmu ? (doc.osoba_odbierajaca || '-') : issuerName(doc)}</b></p>
          <p>Uwagi: <b>{doc.uwagi || '-'}</b></p>
        </div>
      </section>

      <section className="eventflow-pdf-summary mb-8 text-sm">
        <div className="eventflow-pdf-summary-card max-w-sm rounded-xl bg-slate-100 p-4">
          <p className="text-xs font-black uppercase text-slate-500">Waga razem</p>
          <p className="text-2xl font-black">{kg(weightSummary.totalKg)} kg</p>
          <p className="mt-1 text-[10px] font-bold text-slate-500">
            sprzęt {kg(weightSummary.sprzetKg)} kg{weightSummary.caseCount ? ` + case ${kg(weightSummary.caseKg)} kg` : ''}
          </p>
        </div>
      </section>
        ...
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr className="eventflow-pdf-table-head bg-slate-100 text-left">
              ...
              <th className="p-2">Egzemplarz / numer</th>
...
<footer className="mt-10 flex justify-between gap-4 border-t pt-3 text-[10px] font-bold text-slate-400">
  <span>Dokument magazynowy wygenerowano w systemie EVE-nt by Evenement Systems.</span>
  <span>{doc.numer} · {new Date().toLocaleDateString('pl-PL')}</span>
</footer>
    </main>
  </div>;
}
