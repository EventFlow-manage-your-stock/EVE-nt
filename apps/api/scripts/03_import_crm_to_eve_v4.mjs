...
async function main(){
 ...
 // V4: preflight duplikatów NIP w samym pliku.
 // Puste NIP-y są dozwolone, ale dwa niepuste identyczne NIP-y nie przejdą
 // przez lokalny UNIQUE(id_organizacji,nip).
 const nipMap=new Map();
 for(const c of cs){
   const nip=digits(c.nip).slice(0,10);
   if(!nip)continue;
   if(!nipMap.has(nip))nipMap.set(nip,[]);
   nipMap.get(nip).push({key:text(c.__import_key),name:text(c.nazwa)});
 }
 const dupNips=[...nipMap.entries()].filter(([,xs])=>xs.length>1);
 if(dupNips.length){
   console.error('\nDUPLIKATY NIP W CRM_EVE_NT_MERGED.xlsx:');
   for(const [nip,xs] of dupNips.slice(0,50)){
     console.error(`  NIP ${nip}: ${xs.map(x=>`${x.key} "${x.name}"`).join(' | ')}`);
   }
   throw new Error(`Plik zawiera ${dupNips.length} zduplikowanych NIP. Najpierw trzeba poprawić/scalić te firmy.`);
 }
 ...
 const report=await prisma.$transaction(async tx=>{
   ...
   // V4:
   // Lokalna baza może mieć UNIQUE(id_organizacji,nip), mimo że aktualny
   // schema.prisma nie deklaruje już tego constraintu.
   //
   // Przy --replace stare i nowe rekordy muszą przez chwilę współistnieć.
   // Zapamiętaliśmy stare firmy w oldC, więc ich oryginalne NIP-y nadal mamy
   // w pamięci do późniejszego mapowania. Teraz zwalniamy constraint,
   // ustawiając NIP=NULL wyłącznie na STARYCH rekordach.
   //
   // To dzieje się wewnątrz tej samej transakcji. Każdy późniejszy błąd
   // powoduje pełny ROLLBACK również tej operacji.
   if(opts.replace && oldC.length){
     await tx.kontrahent.updateMany({
       where:{id_organizacji:org.id,id:{in:oldC.map(x=>x.id)}},
       data:{nip:null}
     });
     console.log(`REPLACE: tymczasowo zwolniono NIP na ${oldC.length} starych kontrahentach.`);
   }
   ...
   return{
     ...
     contractorRelinked,
     contactRelinked,
     unmatchedContractorRefs,
     unmatchedContactRefs,
     replaceUniqueNipCompatibility:opts.replace
   };
